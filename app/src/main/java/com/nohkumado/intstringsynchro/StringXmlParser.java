...
public class StringXmlParser
{
	...
	private StringEntry readStringArray(XmlPullParser parser) throws IOException, XmlPullParserException
	{
		...
		ArrayList<CdataString> newList = new ArrayList<>();
	 ...
	 */
	private CdataString readItem(XmlPullParser parser) throws IOException, XmlPullParserException
	{
		parser.require(XmlPullParser.START_TAG, ns, "item");
		String summary = "";
		CdataString txt = new CdataString();
		while (parser.next() != XmlPullParser.END_TAG)  getText(parser,txt);
	...
	private StringEntry readPluralItem(XmlPullParser parser) throws IOException, XmlPullParserException
	{
		...
		StringEntry result = new StringEntry(parser.getAttributeValue(null, "quantity"), txt);
	...
	private StringEntry readString(XmlPullParser parser) throws XmlPullParserException, IOException
	{
		...
		if (summary != null) Log.d(TAG, "summary : " + summary);
		...
		// Log.d(TAG,"keeping for "+tag+" : "+summary);

		StringEntry result = new StringEntry(tag, txt);
		//Log.d(TAG, " new string " + result);
		return result;
	}// private StringEntry readString(XmlPullParser parser) throws XmlPullParserException, IOException


	private CdataString getText(XmlPullParser parser, CdataString result) throws XmlPullParserException
	{
		if (result == null) result = new CdataString();
		String summary; 
		...
		{
			//Log.d(TAG, "found TEXT " + summary);
			result.addContent(summary);
		}
		else Log.d(TAG, "Ev type " + parser.getEventType() + " sum:" + summary);
		return result;
	}//private void getText(String summary, XmlPullParser parser) throws XmlPullParserException
	/**
	 * skip unused stuff
	 */
