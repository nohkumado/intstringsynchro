	...
	public void onSelectedFilePaths(String[] selectedStartDirs)
	{
		...
		if (selectedStartDirs.length > 0) 
		{
			...
			for (String fName : selectedStartDirs) 
			{
				...
				ArrayList<StringFile>  fileList = scanner.findStringFiles(fName, error, this);

				for (StringFile aFile: fileList) toLoad.add(aFile);
			}
		...
		else  Toast.makeText(this, R.string.no_dir_found, Toast.LENGTH_LONG).show();
	}
